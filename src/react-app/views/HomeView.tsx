    ...
    >
      ...
      <div className="relative overflow-hidden rounded-3xl bg-[#111111] border border-white/5 shadow-2xl group">
        ...
        <div className="relative z-10 flex flex-col p-8">
          ...
          <h2 className={`font-semibold text-white tracking-tighter mb-8 ${getResponsiveAmountClass(user?.current_balance || 0, 'hero')}`}>
      ...
      <div>
        ...
        {recentTransactions.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-white/10 bg-[#111111]/50 p-8 text-center text-neutral-500">
          ...
          <div className="space-y-3">
            {recentTransactions.map((t, i) => (
              <div 
                ...
                className="group flex items-center justify-between rounded-2xl bg-[#111111] p-5 transition-all hover:bg-[#1a1a1a] border border-white/5 cursor-pointer" 
                style={{ animationDelay: `${i * 100}ms` }}
              >
                <div className="flex items-center gap-4">
                  <div className={`flex h-12 w-12 items-center justify-center rounded-xl bg-black border border-white/5 ${t.transaction_type === 'Incoming' ? 'text-emerald-400' : 'text-red-400'}`}>
